...
const UNIDADES = ['unidad', 'kg', 'atado', 'docena', 'cajón']

export default function NuevaVariedadForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await conCargando(() =>
      supabase.from('lechuga_variedades').insert({
        nombre,
        unidad,
      })
    )
    ...
    if (error) {
      if (error.code === '23505') {
        setError('Ya existe una variedad con ese nombre.')
      } else {
